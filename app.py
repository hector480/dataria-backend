...
def ageb_nse(r: Dict) -> Optional[str]:
    """
    NSE definitivo del AGEB: se clasifica por INGRESO REAL (IXH mensual), no por la
    etiqueta cruda 'NSE PER', que es poco confiable. Si no hay ingreso, cae a la etiqueta.
    """
    nse_income = classify_nse_by_income(_ageb_ixh_mensual(r))
    if nse_income:
        return nse_income
    cls = r.get("NSE PER") or r.get("XI_Nivel socioeconómico por ingreso")
    return cls if cls in NSE_MAP else None
...
def derive_segments(agebs: List[Dict], ft: List[Dict]) -> List[Dict[str, Any]]:
    ...
    evidencia = {i: {"vendidas": 0, "disp": 0, "n": 0} for i in range(len(BUCKETS))}
    for t in ft:
        a = t.get("attributes", {})
    ...
    max_dem = max((demanda_bucket[i]["nuevas_fam"] for i in demanda_bucket), default=0)
    for i, (lo, hi, label) in enumerate(BUCKETS):
        db = demanda_bucket[i]
        if db["nuevas_fam"] <= 0 and db["hogares"] <= 0:
            continue
        ...
        # Status: demanda (nuevas familias) vs oferta disponible + evidencia de venta
        if nuevas == 0:
        ...
        rent_min = round(lo * 1e6 * 0.004)
        rent_max = round((hi if hi < 999 else lo * 1.3) * 1e6 * 0.004)
        segments.append({
            ...
            "status": status, "origen": "demand_driven", "segments_in_bucket": 1,
        })
...
def derive_productos_venta(ft: List[Dict], segments: List[Dict]) -> List[Dict[str, Any]]:
    ...
    for s in segments:
        ...
        # mix %: participación del segmento en el total de nuevas familias
        mix_pct = None
        tot_nf = sum(x["nuevas_fam"] for x in segments if x.get("nuevas_fam")) or 0
        if tot_nf and s.get("nuevas_fam"):
            mix_pct = round(s["nuevas_fam"] / tot_nf * 100)
        # TCA del NSE del segmento
        nse_tca = {"A": 1.12, "B": 1.0, "C+": 1.39, "C": 1.03, "D+": 0.74, "D": 0.37, "E": 0}.get(s["NSE"])
...
def derive_comercio(agebs: List[Dict]) -> Dict[str, Any]:
    ...
    tenant_ideal = {
        ...
        "Entret": "Cine / gimnasio", "Cuidado": "Spa / estética",
        "Mueb": "Mueblería / hogar", "MV": "Ferretería / mejoras",
    }
    renta_m2_cat = {
...
def assemble_zone_payload(req, profile, isocronas, resumen, ft, pagos, resumen_renta,
    ...
    renta_segmentos = derive_renta_segmentos(agebs, nse_dim, ft_renta)
    demanda_segmentos = derive_demanda_segmentos(segments)
    ...
    # Fallback: si no hay oferta de renta levantada, derivar del sweet spot de renta_segmentos
    if (rb_m2 is None or rb_pm2 is None) and renta_segmentos:
        sweet = next((s for s in renta_segmentos if s.get("sweet")), renta_segmentos[0])
        # m² típico del segmento (de la cadena "75-95 m²")
        import re as _re
        nums = _re.findall(r"\d+", sweet.get("m2", ""))
        if rb_m2 is None and nums:
            rb_m2 = round((int(nums[0]) + int(nums[-1])) / 2)
        # renta mensual del segmento (de "$28,500") / m² → pm2
        rnums = _re.findall(r"[\d,]+", sweet.get("renta", "").replace("$", ""))
        if rb_pm2 is None and rnums and rb_m2:
            renta_mensual = int(rnums[0].replace(",", ""))
            rb_pm2 = round(renta_mensual / rb_m2)
    renta_baseline = {
        "m2": rb_m2, "pm2": rb_pm2,
...
def _build_sensibilidad_baseline(productos, segments):
    ...
    # abs base: nuevas familias/mes del segmento (anual/12) como proxy demand-driven
    abs_base = round(base.get("nuevas_fam", 0) / 12, 2) if base.get("nuevas_fam") else None
...
def derive_demanda_segmentos(segments):
    """Tabla simple superior de Demanda (z.demanda_segmentos) derivada de los segments DPO."""
    out = []
    for s in segments:
        if s.get("status") in ("bajo_crecimiento",):
            continue
        out.append({
            "segmento": f"NSE {s['NSE']}",
            "perfil": _status_label(s["status"]),
            "nse": s["NSE"],
            "ticket": (f"${s['val_min']/1e6:.1f}M–${s['val_max']/1e6:.1f}M"
                       if s.get("val_min") and s.get("val_max") else "N/D"),
            "m2": "N/D",
            "abs": round(s["nuevas_fam"], 2) if s.get("nuevas_fam") else "N/D",
            "mix": round(s["mkt_venta"] / max(sum(x["mkt_venta"] for x in segments), 1) * 100)
                   if s.get("mkt_venta") else 0,
        })
    return out


def derive_renta_segmentos(agebs, nse_dim, ft_renta):
    """Segmentos de renta (z.renta_segmentos) por NSE, con V_RENTA real de los AGEBs."""
    if not nse_dim:
        return []
    # Renta promedio por NSE desde V_RENTA ponderada por hogares en alquiler
    rentas_zona = []
    for r in agebs:
        vr = _num(r.get("V_RENTA"))
        if vr is not None and vr > 1000:
            rentas_zona.append(vr)
    renta_med = round(statistics.median(rentas_zona)) if rentas_zona else None

    perfiles = {
        "A": "Alta dirección / capital", "B": "Profesional consolidado",
        "C+": "Joven profesional DINK", "C": "Familia joven", "D+": "Primer empleo",
    }
    out = []
    # Sweet spot de renta: NSE con más hogares en grupos jóvenes/DINK (C+ típicamente)
    best_nse = max(nse_dim, key=lambda n: n["jov_adultos"] + n["consolidados"]) if nse_dim else None
    for n in nse_dim:
        if n["NSE"] not in perfiles or n["hogares"] == 0:
            continue
        # Renta estimada por NSE: proporcional al ingreso del NSE (regla ~25-30% ingreso)
        renta_est = round(n["ixh_nse"] * 0.28) if n.get("ixh_nse") else renta_med
        out.append({
            "seg": f"NSE {n['NSE']}",
            "perfil": perfiles[n["NSE"]],
            "nse": n["NSE"],
            "ing": f"${n['ixh_nse']:,}" if n.get("ixh_nse") else "N/D",
            "renta": f"${renta_est:,}" if renta_est else "N/D",
            "rec": "2 rec" if n["NSE"] in ("C+", "C") else "3 rec",
            "m2": "75-95 m²" if n["NSE"] in ("C+", "C") else "110-140 m²",
            "hog": n["hogares"],
            "sweet": (best_nse is not None and n["NSE"] == best_nse["NSE"]),
        })
    return out
...
async def zona_procesar(req: ZonaRequest):
    ...
    perception = _safe(lambda: value_perception_adjust(resumen, base_ring), "perception",
                       {"n_total": 0, "n_zona": 0, "media": None, "sd": None, "cv": None,
                        ...
                        "motivo": "No disponible", "proyectos": []})
    demografia = _safe(lambda: derive_demografia(agebs), "demografia", derive_demografia([]))
    ...
    productos_renta = _safe(lambda: derive_productos_renta(ft_renta, agebs), "productos_renta", [])
