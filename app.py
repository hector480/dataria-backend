...
def derive_nse_dim(agebs: List[Dict]) -> List[Dict[str, Any]]:
    ...
    for r in agebs:
        ...
        d = agg.setdefault(k, {"poblacion": 0.0, "hogares": 0.0,
                               ...
                               "ingreso_total": 0.0, "valor_viv_total": 0.0})
        ...
        if ing is not None:
            d["ingreso_total"] += ing
        vcasa = _num(r.get("V_CASA")) or _num(r.get("Valor de la vivienda"))
        if vcasa is not None and h:
            d["valor_viv_total"] += vcasa * h
    ...
    for k in order:
        ...
        d = agg.get(k, {"poblacion": 0, "hogares": 0, "ninos": 0, "adolescentes": 0,
                        ...
                        "ingreso_total": 0, "valor_viv_total": 0})
        hog = d["hogares"]
        # ixh_nse: ingreso por hogar real del NSE (si hay dato), si no, punto medio del rango
        if hog and d["ingreso_total"] > 0:
            ixh = round(d["ingreso_total"] / hog)
        else:
            ixh = round((rng["ing_min"] + (rng["ing_max"] or rng["ing_min"] * 1.3)) / 2)
        # viv_nse: precio de vivienda real ponderado (si hay dato), si no, punto medio del rango
        if hog and d["valor_viv_total"] > 0:
            viv = round(d["valor_viv_total"] / hog)
        else:
