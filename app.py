...
def _absorcion_producto(nuevas_fam_year: float, demanda_total: float,
    ...
    if numerador <= 0:
        # Sin demanda en el bucket de precio → no hay base; el producto no se recomienda.
    ...
    if abs_proyecto <= 0:
        # La competencia activa consume toda la demanda mensual → sobreofertado.
...
def _recomendable_por_absorcion(abs_origen: str) -> Dict[str, Any]:
    ...
    # comparables_directos, demanda_sin_competencia u otros con absorción positiva
...
def derive_productos_venta(ft: List[Dict], segments: List[Dict], unidades_proyecto: Optional[int] = None) -> List[Dict[str, Any]]:
    """
...
async def analyze(req: ZonaRequest):
    ...
    productos = derive_productos_venta(ft, segments, req.unidades_proyecto)  # productos venta
    productos_renta = derive_productos_renta(ft_renta, segments)  # productos renta
    comercio = derive_comercio(agebs, _nse_dominante_agebs(agebs)[1], ft)  # potencial comercio/retail (NSE dinámico)
...
async def zona_procesar(req: ZonaRequest):
    ...
    if modo == "vivienda_horizontal":
        ...
        productos = _safe(lambda: derive_productos_venta(ft, segments, req.unidades_proyecto), "productos", [])
        productos_renta = _safe(lambda: derive_productos_renta(ft_renta, segments), "productos_renta", [])
    comercio = _safe(lambda: derive_comercio(agebs, _nse_dominante_agebs(agebs)[1], ft), "comercio", {})
