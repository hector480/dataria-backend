...
def derive_productos_horizontal(ft: List[Dict], segments: List[Dict]) -> List[Dict[str, Any]]:
    ...
    for s in segments:
        ...
        # ABSORCIÓN canónica idéntica a vertical: nuevas_fam/12 × captación (1.0 default).
        nf_month = (s.get("nuevas_fam", 0) or 0) / 12
        abs_rate = nf_month * 1.0
        vend = s.get("evidencia_vendidas", 0) or 0
        n_comp = competencia.get(s["bucket"], 0)
        if (s.get("origen") == "supply_driven" or nf_month == 0):
            abs_evidencia = None
            if vend >= 3:
                abs_evidencia = (vend / 24.0) / max(n_comp, 1)
...
def assemble_zone_payload(req, profile, isocronas, resumen, ft, pagos, resumen_renta,
                          ...
                          agebs=None, ft_renta=None, agebs_geo=None) -> Dict[str, Any]:
    agebs = agebs or []
