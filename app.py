...
def value_perception_adjust(resumen: List[Dict], base_ring: List[List[float]],
                            ...
                            pin_lat: Optional[float] = None) -> Dict[str, Any]:
    """
    ...
    if not mk["barrera"]:
        # Un solo mercado → zona = isócrona completa (regla confirmada).
...
def derive_productos_venta(ft: List[Dict], segments: List[Dict]) -> List[Dict[str, Any]]:
    ...
    for s in segments:
        ...
        # ════════ ABSORCIÓN DEMAND-DRIVEN · FÓRMULA CANÓNICA DEL RUNBOOK ════════
        # Validada en el dashboard de Valle Poniente:
        #   velocity (un/mes) = (nuevas_familias/año ÷ 12) × tasa_de_captación
        # Es el RITMO de nuevas familias del segmento que un proyecto puede captar; NO es
        # absorción de inventario. La PROFUNDIDAD (depth) del segmento son sus hogares; el
        # RITMO (flow) son las nuevas familias/año. La tasa de captación por defecto = 1.0
        # (el front la ajusta con su slider _captureRate; aquí se reporta el máximo demand-driven).
        nf_month = (s.get("nuevas_fam", 0) or 0) / 12   # nuevas familias/mes del segmento
        CAPTURA_DEFAULT = 1.0                            # captación máxima (front la modula)
        abs_rate = nf_month * CAPTURA_DEFAULT

        # Mercado puramente supply-driven (sin demanda DIM local): la absorción se reporta
        # desde la evidencia de venta real del bucket, no desde demanda demográfica.
        ...
        if (s.get("origen") == "supply_driven" or nf_month == 0):
            ...
            if vend >= 3:
                ...
                abs_evidencia = (vend / 24.0) / n_proy_vend   # vendidas en ~24 meses, por proyecto
...
async def zona_procesar(req: ZonaRequest):
    ...
    async with httpx.AsyncClient(timeout=HTTP_TIMEOUT) as client:
        ...
        # VVV (oferta) — universo = anillo MAYOR. Si falla, seguimos con listas vacías
        vvv_venta, vvv_renta = {}, {}
        try:
            vvv_venta = await fetch_vvv(client, outer_ring, "vv_venta")
    ...
    perception = _safe(lambda: value_perception_adjust(resumen, base_ring, req.lng, req.lat), "perception",
    ...
    # El morado (mercado del pin) ya NO recorta el inventario: define el set de
    # competidores DIRECTOS, que se entrega aparte para el análisis competitivo.
    competidores = _safe(
        lambda: clasificar_competidores(resumen, base_ring, outer_ring,
                                        perception.get("zona_poligono")),
        "competidores",
        {"directos": [], "primarios": [], "secundarios": [],
